...
- **Output**: Returns a NumPy array of shape [(Frames, 6, 2)](file:///c:/Users/arvin/.gemini/antigravity/scratch/honors_project_Parkinson-s_disease/all_features/custom_bagging.py#15-52).
...
We executed a 5-Fold Stratified Cross-Validation loop comparing the Native Feature model (using all 8 biomechanical measurements) against the PCA-transformed model using exactly **4 Principal Components**.

### Results
| Metric | Native Features (All 8) | PCA Model (Top 4 Components) | 
|---|---|---|
| **Mean Accuracy** | **99.45%** (± 0.27%) | **97.67%** (± 0.45%) |
| **Mean F1 Score** | **0.9945** (± 0.0028) | **0.9769** (± 0.0044) |

### Conclusion
By leveraging PCA to extract the **4 best latent components**, we halved the computational feature dimensionality (8 dimensional input reduced to 4) while sacrificing **less than 2% in total classification accuracy**. This proves that the core variances separating Parkinsonian gate from healthy gait can be robustly clustered into just four principal components without significant information loss.
